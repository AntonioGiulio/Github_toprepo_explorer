...
@WebServlet("/generateTrendingTopicsDistribution")
public class generateTrendingTopicsDistribution extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
	
	public generateTrendingTopicsDistribution() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		boolean lockResourceTrendingTopic=false;
		if(s.getAttribute("lockResourceTrendingTopic")!=null) {
			System.out.println("Inizio -> Attributo in sessione:" + s.getAttribute("lockResourceTrendingTopic"));
		}
		
		System.out.println("Entro nel while -> Attributo in sessione:" + s.getAttribute("lockResourceTrendingTopic"));
		while(true) {
			if(s.getAttribute("lockResourceTrendingTopic") != null) {
				lockResourceTrendingTopic = (boolean) s.getAttribute("lockResourceTrendingTopic");
			}
			
			//se la risorsa è occupata (lockResource=true) aspetto 60 secondi, altrimenti esco dal while e procedo con l'esecuzione
			if(lockResourceTrendingTopic) {
				try {
				    Thread.sleep(60000);
				} catch(InterruptedException e) {
					s.setAttribute("lockResourceTrendingTopic", false);
		...
		System.out.println("Esco dal while -> Attributo in sessione:" + s.getAttribute("lockResourceTrendingTopic"));
		...
		//otteniamo la lista
		try {
			s.setAttribute("lockResourceTrendingTopic", true);
			System.out.println("Eseguo API -> Attributo in sessione:" + s.getAttribute("lockResourceTrendingTopic"));
			list = reqGen.searchByRecentTrendingTopics();
			s.setAttribute("lockResourceTrendingTopic", false);
		} catch (Exception e) {
			s.setAttribute("lockResourceTrendingTopic", false);
			e.printStackTrace();
		...
		*/
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("/pages/trendingTopicsDistribution.jsp");
