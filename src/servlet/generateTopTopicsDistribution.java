...
@WebServlet("/generateTopTopicsDistribution")
public class generateTopTopicsDistribution extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
	
	public generateTopTopicsDistribution() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		boolean lockResourceTopic=false;
		if(s.getAttribute("lockResourceTopic")!=null) {
			System.out.println("Inizio -> Attributo in sessione:" + s.getAttribute("lockResourceTopic"));
		}
		
		System.out.println("Entro nel while -> Attributo in sessione:" + s.getAttribute("lockResourceTopic"));
		while(true) {
			if(s.getAttribute("lockResourceTopic") != null) {
				lockResourceTopic = (boolean) s.getAttribute("lockResourceTopic");
			}
			
			//se la risorsa è occupata (lockResource=true) aspetto 60 secondi, altrimenti esco dal while e procedo con l'esecuzione
			if(lockResourceTopic) {
				try {
				    Thread.sleep(60000);
				} catch(InterruptedException e) {
					s.setAttribute("lockResourceTopic", false);
		...
		System.out.println("Esco dal while -> Attributo in sessione:" + s.getAttribute("lockResourceTopic"));
		...
		//otteniamo la lista
		try {
			s.setAttribute("lockResourceTopic", true);
			System.out.println("Eseguo API -> Attributo in sessione:" + s.getAttribute("lockResourceTopic"));
			list = reqGen.searchByTopTopics();
			s.setAttribute("lockResourceTopic", false);
		...
		*/
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("/pages/topTopicsDistribution.jsp");
